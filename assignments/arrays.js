...
let oldCars = [];
for (let i = 0; i < inventory.length; i++) {
    var current_car_year = inventory[i].car_year;
    if (current_car_year < 2000) {
        oldCars.push(current_car_year);
    }
}
...
let BMWAndAudi = [];
for (let i = 0; i < inventory.length; i++) {
    var current_car_make = inventory[i].car_make;
    if (current_car_make == ("BMW" || "Audi")) {
